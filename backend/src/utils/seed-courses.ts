...
async function main() {
  console.log('--- Seeding demo courses + enrollments ---');
  console.log('Database URI from env config:', env.MONGODB_URI);

  // Đảm bảo có instructor & student
  const instructor = await prisma.user.findUnique({
    where: { email: 'instructor@noblecert.com' },
  });
  if (!instructor) {
    throw new Error('Không tìm thấy instructor với email instructor@noblecert.com. Hãy chạy seed-users trước.');
  }
  ...
  if (!student) {
    throw new Error('Không tìm thấy student với email student@example.com. Hãy chạy seed-users trước.');
  }
      ...
      units: [
        {
          ...
          type: 'video',
          contentUrl: 'https://www.youtube.com/embed/X3paOmcrTjQ',
          duration: 600,
        },
        {
          unitId: 'ds-unit-2',
      ...
      units: [
        {
          ...
          type: 'video',
          contentUrl: 'https://www.youtube.com/embed/Pj15Ie_C-0k',
          duration: 900,
        },
      ],
    },
  ...
  console.log('--- Hoàn tất seeding courses ---');
}

main()
  .catch((e) => {
    console.error('❌ Lỗi khi seeding courses:', e);
    process.exit(1);
  })
  .finally(async () => {
