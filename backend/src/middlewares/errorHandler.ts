...
export const errorHandler = (err: any, req: Request, res: Response, next: NextFunction) => {
  ...
  // Multer upload errors (file size/type)
  if (err?.name === 'MulterError') {
    if (err.code === 'LIMIT_FILE_SIZE') {
      error = new AppError('File quá lớn (tối đa 5MB)', 400);
    } else {
      error = new AppError('Upload file không hợp lệ', 400);
    }
  }
  // fileFilter() thường throw Error thường → map thành 400 để hiển thị message
  if (typeof err?.message === 'string' && err.message.includes('Chỉ cho phép upload file ảnh')) {
