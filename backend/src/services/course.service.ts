...
export class CourseService {
   ...
   */
  static async getPublishedCourses(limit = 20, filters?: { search?: string; topic?: string; goal?: string }) {
    ...
    // Filter by topic/goal in-memory (stored in settings.category as JSON)
    if (filters?.topic) {
      const topicLower = filters.topic.toLowerCase();
      results = results.filter((c) =>
        c.category?.toLowerCase().includes(topicLower) ||
        c.name.toLowerCase().includes(topicLower) ||
        c.description.toLowerCase().includes(topicLower)
      );
    }

    if (filters?.goal === 'free') {
      results = results.filter((c) => !c.price || Number(c.price) === 0);
    }
