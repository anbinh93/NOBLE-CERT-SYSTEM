...
export const getPublicCourses = catchAsync(async (req: Request, res: Response) => {
  ...
  const courses = await CourseService.getPublishedCourses(limit, { search, topic, goal });
...
async function findEnrollmentBySerial(serial: string) {
  ...
  const ids = completedEnrollments.map((e) => e.id);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const revocations: any[] = ids.length
  ...
  return (
    completedEnrollments.find(
      (e) => e.id.slice(-8).toUpperCase() === hash && e.attempts[0]?.isPassed && !revokedSet.has(e.id),
    ) ?? null
  );
}
