...
export class LearningController {
  ...
  static async submitExam(req: Request | any, res: Response) {
    ...
    const data = await LearningService.submitExam(req.user.id, courseId, answers);
...
export async function getCourseContent(req: Request, res: Response) {
  ...
  const settings = (course.settings as Record<string, unknown>) ?? {};
  const totalUnits = units.filter((u: any) => typeof u.type !== 'string' || u.type.toLowerCase() !== 'exam').length;
  const completedUnits = enrollment.progresses
    .filter((p: any) => p.isCompleted)
    .map((p: any) => p.unitId);
  const progress = totalUnits > 0 ? Math.round((completedUnits.length / totalUnits) * 100) : 0;
  ...
  for (const unit of units) {
    ...
    sectionMap.get(sid).units.push({
      _id: unit.unitId,
      title: unit.title,
      type: unit.type?.toUpperCase() ?? 'DOCUMENT',
...
export async function getCourseStatus(req: Request, res: Response) {
  ...
  const emptyStatus = { isEligible: false, missingRequirements: [], examScore: 0, isEligibleForCertificate: false, progress: 0 };
  ...
  const progress = totalRequired > 0 ? Math.round((completedUnitIds.length / totalRequired) * 100) : 0;

  res.status(200).json({
    status: 'success',
    data: { isEligible, missingRequirements, examScore, isEligibleForCertificate, progress },
  });
}
