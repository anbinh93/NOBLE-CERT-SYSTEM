                 ...
                 {enrollments.map((enrollment: any) => {
                     ...
                     <div key={enrollment._id} className="group bg-card rounded-[24px] p-5 shadow-sm border border-border hover:shadow-xl transition-all hover:-translate-y-1">
                         <div className="aspect-[16/9] bg-muted rounded-2xl mb-4 relative overflow-hidden">
                              {course.thumbnail ? (
                                <FallbackImage src={course.thumbnail} alt={course.name} fill className="object-cover" />
                              ) : (
                              ...
                              {/* Overlay Play Button */}
                              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-foreground/20 backdrop-blur-[2px]">
