...
export default function ExamPage() {
  ...
  // ── Load exam ─────────────────────────────────────────────────────────────
  const loadExam = async (): Promise<boolean> => {
    if (!accessToken) { alert("Phiên đăng nhập hết hạn."); return false; }
    setLoadingExam(true);
    try {
      const data = await learningService.getExam(courseId, accessToken);
      setExamData(data as { passingScore?: number; duration?: number });
      setQuestions(normalizeQuestions((data as { questions?: unknown[] }).questions ?? []));
      setTimeLeft((data as { duration?: number }).duration ?? 2700);
      return true;
    } catch {
      return false;
    } finally {
  ...
  const submitExam = useCallback(async () => {
    ...
    if (!accessToken) { alert("Phiên đăng nhập hết hạn."); return; }
    setSubmitting(true);
    try {
  ...
  if (examState === "INTRO") {
    ...
    return (
      <div className="h-full overflow-y-auto bg-background p-4 md:p-8 flex items-center justify-center">
        <div className="max-w-2xl w-full bg-card rounded-[32px] p-8 md:p-12 shadow-2xl border border-primary/20 text-center relative overflow-hidden">
          ...
          {!isEligible ? (
            <div className="bg-orange-50 dark:bg-orange-950/30 text-orange-800 dark:text-orange-200 p-6 rounded-2xl mb-8 border border-orange-200/50 text-left">
              ...
              <ul className="space-y-2 bg-white/40 dark:bg-black/20 p-3 rounded-xl">
      ...
      <div className="h-full overflow-y-auto bg-background p-4 md:p-8 flex items-center justify-center">
        <div className="max-w-lg w-full bg-card rounded-3xl p-10 shadow-xl border border-primary/20 text-center relative overflow-hidden">
          ...
          <div className={cn(
            ...
            passed
              ? "bg-green-100 dark:bg-green-900/30 text-green-600 ring-green-500"
