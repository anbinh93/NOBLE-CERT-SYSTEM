...
function LearningLayoutContent({ children, params }: { children: React.ReactNode, params: any }) {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC]">
                <div className="text-center space-y-4">
                    <p className="text-lg font-medium text-slate-700">Bạn chưa đăng ký khoá học này</p>
                    <p className="text-sm text-slate-500">Vui lòng đăng ký trước khi truy cập nội dung.</p>
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-64px)] bg-[#F8FAFC] text-slate-800 font-sans">
             <div className="flex-1 flex overflow-hidden">
                {/* LEFT SIDEBAR - PERSISTENT */}
                <aside 
                    className={cn(
                        "flex-shrink-0 bg-white border-r border-gray-200 flex flex-col transition-all duration-300 z-20 absolute md:relative h-full",
                        sidebarOpen ? "w-80 translate-x-0" : "w-0 -translate-x-full opacity-0 overflow-hidden"
                    )}
                >
                    <div className="flex flex-col border-b border-gray-100 bg-white z-10 p-4">
                        <Link href={`/student/learn/${params.courseId}`} className="group flex items-center gap-3 px-2 py-2 text-sm font-medium text-slate-600 hover:text-blue-600 transition-colors mb-4 rounded-lg hover:bg-blue-50">
                            <div className="p-1 bg-gray-100 group-hover:bg-blue-100 rounded-md transition-colors">
                                <ArrowLeft className="w-4 h-4" />
                            </div>
                            Quay lại tổng quan
                        </Link>
                        <h2 className="font-bold text-slate-900 leading-tight px-2">{course.name}</h2>
                        
                        <div className="flex items-center gap-2 mt-4 px-2 text-xs text-slate-500">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar bg-white p-4 space-y-4">
                                    ...
                                    <div key={sIdx} className="bg-white">
                                        <button 
                                            ...
                                            className={cn(
                                                ...
                                                isSectionLocked ? "opacity-50 cursor-not-allowed" : "hover:text-blue-600"
                                            )}
                                        >
                                            <div className="flex items-center gap-2 flex-1 min-w-0">
                                                {isSectionLocked && <div className="w-4 h-4 rounded-full bg-gray-100 flex items-center justify-center"><Lock className="w-2.5 h-2.5 text-gray-400" /></div>}
                                                <h4 className={cn("text-sm font-bold truncate", isSectionLocked ? "text-slate-400" : "text-slate-800 group-hover:text-blue-600")}>
                                                    {section.title}
                                                </h4>
                                            </div>
                                            {!isSectionLocked && (isExpanded ? <ChevronDown className="w-4 h-4 text-slate-400" /> : <ChevronRight className="w-4 h-4 text-slate-400" />)}
                                        </button>
                                        ...
                                        {!isSectionLocked && isExpanded && (
                                            <div className="mt-1 space-y-1 pl-2">
                                                {section.units?.map((unit, uIdx) => {
                                                    ...
                                                    if (isUnitLocked) {
                                                         return (
                                                            <div key={uIdx} className="flex items-start gap-3 px-3 py-2.5 rounded-lg text-sm text-slate-400 cursor-not-allowed select-none opacity-50">
                                                        ...
                                                        >
                                                            <div className="mt-0.5 flex-shrink-0">
                                                                {isCompleted ? (
                                                                    ...
                                                                    unit.type === "VIDEO" ? <PlayCircle className={cn("w-4 h-4", isActive ? "text-blue-600" : "text-slate-400")} /> : <FileText className={cn("w-4 h-4", isActive ? "text-blue-600" : "text-slate-400")} />
                                                                )}
                                                            </div>
                                                            <span className="leading-snug">{unit.title}</span>
                        ...
                        <div className="bg-white">
                            <button 
                                ...
                                className="w-full flex items-center justify-between py-2 text-left hover:text-blue-600 transition-colors group"
                            >
                                <h4 className="text-sm font-bold text-slate-800 group-hover:text-blue-600 flex-1">Bài thi cuối khóa</h4>
                                {expandedSections.includes(999) ? <ChevronDown className="w-4 h-4 text-slate-400" /> : <ChevronRight className="w-4 h-4 text-slate-400" />}
                            </button>
                                            ...
                                            <Link 
                                                ...
                                                className={cn(
                                                    ...
                                                    pathname.includes("/exam") 
                                                        ? "bg-blue-50 text-blue-700 font-medium" 
                                                        : isExamLocked ? "text-slate-400 opacity-50 cursor-not-allowed" : "text-slate-600 hover:bg-gray-50"
                                                )}
                                                ...
                                                <div className="mt-0.5 flex-shrink-0">
                                                    {isExamPassed ? (
                                                        ...
                                                        <Lock className="w-4 h-4 text-slate-300" />
                                                         ...
                                                         <FileText className={cn("w-4 h-4", pathname.includes("/exam") ? "text-blue-600" : "text-slate-400")} />
                                                    )}
                                                </div>
                                                <span className="leading-snug">Bài thi trắc nghiệm</span>
                                                {pathname.includes("/exam") && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-blue-600 rounded-r-full"></div>}
                                            </Link>
                                         );
                                     })()}
                                     ...
                                     {(() => {
                                         ...
                                         return (
                                            <Link 
                                                ...
                                                className={cn(
                                                    ...
                                                    pathname.includes("/claim") 
                                                        ? "bg-blue-50 text-blue-700 font-medium" 
                                                        : isCertLocked ? "text-slate-400 opacity-50 cursor-not-allowed" : "text-slate-600 hover:bg-gray-50"
                                                )}
                                            ...
                                            >
                                                ...
                                                {pathname.includes("/claim") && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-blue-600 rounded-r-full"></div>}
                                            </Link>
                                         )
                                     })()}
                                </div>
                ...
                <main className="flex-1 flex flex-col overflow-hidden relative w-full bg-[#FAFBFF]">
                    {/* Mobile Toggle */}
                     <div className="md:hidden h-12 border-b border-gray-200 flex items-center px-4 bg-white flex-shrink-0">
                        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-1">
                            {sidebarOpen ? <X className="w-5 h-5"/> : <Menu className="w-5 h-5"/>}
                        </button>
                        <span className="ml-3 font-bold text-sm text-slate-900 truncate">{course.name}</span>
                     </div>
                     ...
                     <div className="flex-1 flex flex-col relative w-full h-full overflow-hidden">
                         ...
                         <button 
                             ...
                             className="hidden md:flex absolute top-4 left-4 z-30 w-8 h-8 items-center justify-center bg-white border border-gray-200 rounded-full shadow-sm text-slate-500 hover:text-blue-600 hover:shadow-md transition-all"
