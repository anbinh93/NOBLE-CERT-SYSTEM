...
export default function ClaimCertificatePage() {
    ...
    const handleCreatePayment = async () => {
        ...
        try {
            ...
            if (data.qrCode || data.checkoutUrl) {
                ...
                toast.error(json.message || "Không tạo được link thanh toán");
            }
        } catch {
    ...
    const handleConfirmPayment = async () => {
        ...
        try {
            ...
            if (data.isPaid) {
                ...
                toast.warning(data.message || "Chưa xác nhận được thanh toán, vui lòng thử lại.");
            }
        } catch {
    ...
    const handleShare = () => {
        if (certificateSerial) {
            ...
            toast.error("Không tìm thấy mã serial chứng chỉ.");
        }
    };
    ...
    const typedStatusData = (statusData ?? {}) as CourseStatusPayload;
    const isEligibleForCertificate = typedStatusData.isEligibleForCertificate;
    const examScore = typedStatusData.examScore ?? 0;
    const passingScore = typedStatusData.passingScore ?? 80;
    const paymentAmount = paymentState.amount
      ? new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(paymentState.amount)
      : defaultPriceString;
            ...
            <div className="min-h-screen bg-background flex justify-center py-12 px-4">
                 <div className="max-w-md w-full text-center space-y-6 pt-20">
                     ...
                     <p className="text-muted-foreground text-lg">
                        Bạn cần hoàn thành bài kiểm tra cuối khóa với điểm số trên <b className="text-foreground">{passingScore}%</b> để nhận chứng chỉ.
                     </p>
                     
                     <div className="bg-card p-6 rounded-2xl shadow-sm border border-border">
                         <div className="flex justify-between items-center mb-2">
                             <span className="text-sm font-bold text-muted-foreground uppercase">Điểm hiện tại</span>
                            <span className={cn("text-2xl font-bold", examScore >= passingScore ? "text-green-600 dark:text-green-400" : "text-destructive")}>{examScore}%</span>
                         </div>
                         <div className="w-full h-2 bg-secondary rounded-full overflow-hidden">
                            <div className={cn("h-full rounded-full", examScore >= passingScore ? "bg-primary" : "bg-destructive")} style={{ width: `${Math.min(100, examScore)}%` }}></div>
                         </div>
                     </div>
                ...
                <div className="bg-card rounded-[32px] overflow-hidden shadow-sm border border-primary/10 flex flex-col lg:flex-row min-h-[600px]">
                    ...
                    <div className="lg:w-1/2 p-10 md:p-14 flex flex-col justify-center bg-card relative">
                        <div className="relative z-10">
                            {/* Header */}
                            <div className="mb-8">
                                <div className="inline-flex items-center gap-2 bg-primary/10 text-primary px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-4 border border-primary/20">
                                    ...
                                    <div className="bg-primary/5 rounded-2xl p-6 border border-primary/20 border-l-4 border-l-primary">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            <Link href={`/verify/${certificateSerial}`} target="_blank" className="w-full">
                                                <Button className="w-full bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl shadow-lg h-12 font-bold text-base">
                                                    <Zap className="w-5 h-5 mr-2" /> Xem chứng chỉ trực tuyến
                                                </Button>
                                            </Link>
                                            <div className="flex gap-3">
                                                <Button onClick={() => handleDownload('png')} variant="outline" className="flex-1 border-primary/30 text-primary hover:bg-primary/10 rounded-xl">
                                                    <Download className="w-4 h-4 mr-2" /> Tải ảnh
                                                </Button>
                                                <Button onClick={handleShare} variant="outline" className="flex-1 border-primary/30 text-primary hover:bg-primary/10 rounded-xl">
                                    ...
                                    <div className="bg-secondary rounded-2xl p-6 border border-primary/20 animate-in fade-in zoom-in-95">
                                         ...
                                         <div className="flex flex-col md:flex-row items-center gap-6">
                                             <div className="bg-card p-2 rounded-xl border-dashed border-2 border-border">
                                                 {paymentState.qrCode
                                                    ? <img src={`https://api.qrserver.com/v1/create-qr-code/?size=128x128&data=${encodeURIComponent(paymentState.qrCode)}`} alt="QR Code thanh toán" width={128} height={128} className="rounded-lg" />
                                             ...
                                             <div className="flex-1 w-full text-center md:text-left">
                                                 ...
                                                 <p className="text-2xl font-bold text-primary mb-3">{paymentAmount}</p>
                                    ...
                                    <div className="bg-card rounded-2xl p-6 border border-primary/10 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                                        <div className="flex justify-between items-end mb-6">
                                            ...
                                            <div className="text-right">
                                                <span className="block text-3xl font-serif font-bold text-foreground">{paymentAmount}</span>
                                            </div>
                                        </div>
                                        <Button 
                         ...
                         <div className="relative w-full max-w-lg aspect-[1.414/1] shadow-2xl shadow-foreground/10 rotate-1 hover:rotate-0 transition-all duration-700 group">
                             ...
                             {certificateSerial ? (
                                 <div className="relative w-full h-full bg-card rounded-[20px] overflow-hidden">
                                     ...
                                     {/* Overlay Shine */}
                                     <div className="absolute inset-0 bg-linear-to-tr from-transparent via-white/20 to-transparent pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-1000 -translate-x-full group-hover:translate-x-full" style={{ transition: 'transform 1.5s ease-in-out' }}></div>
                                 </div>
                             ) : (
                                 /* Fallback / Mock Preview */
                                <div className="relative w-full h-full bg-card rounded-[20px] border-12 border-card ring-1 ring-border flex items-center justify-center">
