...
const BANK_ID = "MB"; // Example Bank
const ACCOUNT_NO = "0000123456789"; // Example Account
const ACCOUNT_NAME = "CERTIFLOW EDUCATION";
...
export default function CertificateModal({ course, onClose, userId }: CertificateModalProps) {
  ...
  const simulatePayment = async () => {
      ...
      toast.info("Đang giả lập thanh toán thành công...");
      setStep(3);
  };
    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-card rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 border border-border">
          ...
          <div className="p-4 border-b border-border flex justify-between items-center bg-muted/50">
              ...
              <button onClick={onClose} className="p-1 hover:bg-muted rounded-full text-foreground">
                  <X size={20} />
              </button>
          </div>
          ...
          <div className="p-6">
              {step === 1 && (
                  <div className="space-y-4">
                      <div className="border border-border rounded-lg p-4 bg-muted/30 text-center">
                          <div className="text-muted-foreground text-sm">Demo Chứng chỉ</div>
                          <div className="font-serif text-xl font-bold text-primary mt-2">{course.courseTitle}</div>
                          <div className="text-muted-foreground mt-1">Cấp cho: Student Name</div>
                      </div>
                      
                      <div className="flex justify-between items-center text-lg">
                          <span className="text-foreground">Phí cấp bằng:</span>
                          <div className="text-right">
                              <span className="block font-bold text-destructive">199.000đ</span>
                              <span className="text-sm text-muted-foreground line-through">500.000đ</span>
                          </div>
                      </div>
                      ...
                      >
                          {loading ? <Loader2 className="animate-spin" /> : "THANH TOÁN & NHẬN BẰNG"}
                      </button>
                  </div>
              )}
              ...
              {step === 2 && order && (
                   <div className="text-center space-y-4">
                       ...
                       <div className="mx-auto w-64 h-64 bg-card rounded-lg overflow-hidden border border-border">
                           {/* Dynamic VietQR */}
                           <img 
                       ...
                       <div className="bg-primary/10 p-3 rounded text-sm text-primary">
                           Nội dung CK: <strong className="font-mono text-lg">{order.orderCode}</strong>
                       </div>
                        ...
                        {/* DEMO BUTTON */}
                        <button onClick={simulatePayment} className="text-xs text-muted-foreground/50 underline mt-4 hover:text-foreground">
              ...
              {step === 3 && (
                  <div className="text-center py-8 space-y-4">
                      <div className="mx-auto w-20 h-20 bg-primary/10 rounded-full flex items-center justify-center text-primary mb-4 animate-bounce">
                          <CheckCircle size={40} />
                      </div>
                      <h4 className="text-2xl font-bold text-foreground">Thanh toán thành công!</h4>
                      <p className="text-muted-foreground">Chứng chỉ của bạn đã sẵn sàng.</p>
