...
// "Tài khoản" section trong nav — quản lý toàn bộ user (tất cả role)
'use client';
...
const ROLE_BADGE: Record<string, { label: string; className: string }> = {
  ...
  STAFF: { label: 'Nhân viên', className: 'bg-accent text-accent-foreground border-accent/50' },
  INSTRUCTOR: { label: 'Giảng viên', className: 'bg-secondary text-secondary-foreground border-secondary/50' },
  STUDENT: { label: 'Học viên', className: 'bg-muted text-muted-foreground border-border' },
};

function useAdminUsers(filters: { page?: number; pageSize?: number; q?: string; role?: string }) {
  return useQuery({
    queryKey: ['admin', 'users', filters],
    queryFn: () =>
      adminApi.get<PaginatedResult<AdminUser>>(`/users${buildQuery({ ...filters })}`),
  });
}

export default function UsersPage() {
  const [search, setSearch] = useState('');
  const [filters, setFilters] = useState({ page: 1, pageSize: 20, q: '', role: '' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {data ? `${new Intl.NumberFormat('vi-VN').format(data.total)} tài khoản` : 'Quản lý toàn bộ người dùng hệ thống'}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <Button
          variant="outline" size="icon" className="rounded-full border-border"
          onClick={() => refetch()} aria-label="Làm mới danh sách"
        >
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border hover:bg-transparent">
              ...
              <TableHead className="text-muted-foreground font-semibold hidden sm:table-cell">Vai trò</TableHead>
              <TableHead
                className="text-right text-muted-foreground font-semibold hidden lg:table-cell cursor-pointer select-none"
          ...
          <TableBody>
            ...
            {error && (
              <TableRow className="border-border">
                <TableCell colSpan={6} className="text-center py-12 text-muted-foreground">
                  Lỗi tải dữ liệu.{' '}
                  <button className="text-primary underline" onClick={() => refetch()}>Thử lại</button>
                </TableCell>
              </TableRow>
            )}
            ...
            {sortedItems.map((user) => {
              const roleName = (user as AdminUser & { role?: string }).role ?? 'STUDENT';
              const roleBadge = ROLE_BADGE[roleName] ?? ROLE_BADGE.STUDENT;
              return (
                <TableRow key={user.id} className="border-border hover:bg-muted/30 transition-colors duration-150">
                  <TableCell>
                  ...
                  <TableCell className="text-right hidden lg:table-cell">
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      <span className="text-foreground font-medium">{user._count.enrollments}</span>
                    </div>
                  </TableCell>
